...
export default function Home() {
  ...
  // machine à écrire + retours à la ligne
  useEffect(() => {
    if (!lore) return;
  ...
  // lock scroll quand la popup est ouverte (tous OS)
  useEffect(() => {
  ...
  return (
    <>
      ...
      {/* MODAL — rendu inline, z-index maximum */}
      {showPopup && (
        <div
          className="fixed inset-0 z-[9999] bg-black/70 flex items-start justify-center overflow-y-auto"
          onClick={() => setShowPopup(false)}
        >
          <div className="w-[92vw] max-w-md md:max-w-xl p-2 sm:p-4">
            <div
              className="relative bg-gray-900 text-white rounded-lg shadow-2xl"
              style={{ marginTop: 'max(env(safe-area-inset-top), 6px)' }}
              role="dialog"
              aria-modal="true"
              onClick={(e) => e.stopPropagation()} // bloque la fermeture si on clique dans la boîte
            >
              <button
